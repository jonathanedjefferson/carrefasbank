...
export default class BCSF_CancelamentoConta extends LightningElement {
    //#region Variaveis 
    spinner = false;
    logoTipo;
    optionsStatus = [];
    ...
    verifyDesable() {
        const isFalecimento = this.valueEvento === 'Falecimento';
        const isOrigemPortalOuLojas = this.valueOrigem === 'Portal' || this.valueOrigem === 'Lojas';

        if (
            this.valueStatus == null || this.valueStatus == undefined ||
            ((!isFalecimento) && (this.valueMotivo == null || this.valueMotivo == undefined)) ||
            this.valueOrigem == null || this.valueOrigem == undefined ||
            ((!isOrigemPortalOuLojas) && (this.valueCanal == null || this.valueCanal == undefined)) ||
    ...
    carregarListaStatus() {
        ...
        getlistStatusCancelamento({
            ...
            if (result.status.length > 0) {
                ...
                this.showToast('Erro', 'Erro ao buscar lista de status para Cancelar!', 'error', true);
            }
        }).catch(error => {
    ...
    CancelamentoConta() {
        ...
        cancelamentoConta({
            ...
            if (result) {
                ...
                this.showToast('Erro', 'Não foi possivel bloquear a Conta!', 'error', true);
            }
